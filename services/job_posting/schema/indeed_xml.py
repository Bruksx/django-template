...
class JobBase:
    ...
    def convert_to_job(cls, job_post):
        ...
        return JobBase(
                ...
                experience=f"{job.years_of_experience} years of experience" if job.years_of_experience else job.years_of_experience,
...
class Source:
    ...
    def to_xml_stream():
        ...
        for job_post in JobPost.objects.select_related("job").\
            filter(status=JobStatusType.POSTED.value).order_by("-created_at").iterator(chunk_size=30):

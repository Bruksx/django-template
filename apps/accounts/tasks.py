...
def remind_incomplete_profiles_task(batch_size=200):
    ...
    for batch in range(batches):
        emails = list(User.objects.filter(talent_id__in=talent_ids[(batch * batch_size):((batch + 1) * batch_size)]).only("email").values_list("email", flat=True))
        send_incomplete_profile_reminder_email(emails=emails)
    if remainder > 0:
        emails = list(User.objects.filter(talent_id__in=talent_ids[-remainder:]).only(
